...
public class GuardianMonth extends Base {
  int payment_per_monthId;
  HSSFWorkbook workbook;
  ExcelUtil eu;
  
  public GuardianMonth(String templateName, int payment_per_monthId)throws IOException {
    super();
  ...
  public String getQueryString() {
    String sql = ""
    + "SELECT a.*, b.*, c.*, d.*, c.temporary_payment_flg as exd_temporary_payment_flg FROM payment_per_months a                 \n"
  ...
  public void procResultSet(ResultSet res) throws SQLException {
    ...
    while(res.next()) {
      ...
      //仮払金合計
      if ((res.getLong("temporary_flg") == 1) && (res.getLong("exd_temporary_payment_flg") == 1))
      { 
      	temporary_total += res.getLong("amount"); 
      }
      else if ((res.getLong("temporary_flg") == 1) && (res.getLong("exd_temporary_payment_flg") == 0)) 
      { 
      	countTemNoPayment += 1; 
      }
      //経費合計
      else if (res.getLong("temporary_flg") == 0) 
      {
        all_total += res.getLong("amount");
      }
      
      if ((res.getLong("temporary_flg") == 0) || ((res.getLong("temporary_flg") == 1) && (res.getLong("exd_temporary_payment_flg") == 1)))
      {
    	procPaymentPerMonthDetail(res, curSheetTotal, ExcelUtil.HEADER_ROW_COUNT + index - countTemNoPayment);
      }
  	...
  	if (index > 0) {
      if ((index - countTemNoPayment) > ExcelUtil.DETAIL_ROW_COUNT) { startFooterRow = ExcelUtil.HEADER_ROW_COUNT + index + 1 - countTemNoPayment; }
  ...
  public void procPaymentPerMonthHeader(ResultSet res, HSSFSheet sheet) throws SQLException {
    if (res.getDate("cutoff_end_date") != null) eu.getCell(sheet,0,0).setCellValue((new SimpleDateFormat("yyyy年MM月")).format(res.getDate("cutoff_end_date")) + "度 月次精算");
  ...
  public void procSummaryPaymentPerMonthHeader(ResultSet res, HSSFSheet sheet) throws SQLException {
  	if (res.getDate("cutoff_end_date") != null) eu.getCell(sheet,0,0).setCellValue((new SimpleDateFormat("yyyy年MM月")).format(res.getDate("cutoff_end_date")) + "度 月次精算");
  ...
  public void procPaymentPerMonthDetail(ResultSet res, HSSFSheet sheet, int row) throws SQLException {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      Base obj = new GuardianMonth(template, id);
