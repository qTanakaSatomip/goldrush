...
public class GuardianCase extends Base {
  int payment_per_caseId;
  HSSFWorkbook workbook;
  ExcelUtil eu;
  
  public GuardianCase(String templateName, int payment_per_caseId)throws IOException {
    super();
    this.payment_per_caseId = payment_per_caseId;
  ...
  public String getQueryString() {
    String sql = ""
    + "SELECT * FROM payment_per_cases a                 \n"
  ...
  public void procResultSet(ResultSet res) throws SQLException {
    ...
    while(res.next()) {
      ...
      procPaymentPerCaseDetail(res, curSheetTotal, ExcelUtil.HEADER_ROW_COUNT + index);
      index += 1;
  ...
  public void procPaymentPerCaseHeader(ResultSet res, HSSFSheet sheet) throws SQLException {
    if (res.getDate("cutoff_end_date") != null) eu.getCell(sheet,0,0).setCellValue((new SimpleDateFormat("yyyy年MM月")).format(res.getDate("cutoff_end_date")) + "度 都度精算");
  ...
  public void procSummaryPaymentPerCaseHeader(ResultSet res, HSSFSheet sheet) throws SQLException {
  	if (res.getDate("cutoff_end_date") != null) eu.getCell(sheet,0,0).setCellValue((new SimpleDateFormat("yyyy年MM月")).format(res.getDate("cutoff_end_date")) + "度 都度精算");
  ...
  public void procPaymentPerCaseDetail(ResultSet res, HSSFSheet sheet, int row) throws SQLException {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      Base obj = new GuardianCase(template, id);
